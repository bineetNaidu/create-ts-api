...
export class TweetController {
  ...
  public getTweetById = async (req: Request, res: Response): Promise<void> => {
    const { id } = req.params;
    const tweet = await this.tweetService.getTweetById(id as string);
  ...
  public updateTweet = async (req: Request, res: Response): Promise<void> => {
    const { id } = req.params;
    const tweet = await this.tweetService.updateTweet(id as string, req.body);
    res.status(200).json({ data: tweet });
  };

  public deleteTweet = async (req: Request, res: Response): Promise<void> => {
    const { id } = req.params;
    await this.tweetService.deleteTweet(id as string);
    res.status(204).send();
  };
}
